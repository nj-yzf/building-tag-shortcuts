...
- `Ctrl+Shift+Q`: set or update `height=3.6*building:levels` on selected objects that already have a parseable positive `building:levels` value.
...
- Fill or update `height` from existing `building:levels` using `height=3.6*n`.
...
- 2026-05-10: Added `Ctrl+Shift+Q` height-from-levels feature.
