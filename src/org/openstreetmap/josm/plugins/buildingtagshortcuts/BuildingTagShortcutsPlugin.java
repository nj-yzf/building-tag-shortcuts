...
public class BuildingTagShortcutsPlugin extends Plugin {
    ...
    private static ApplyHeightAction applyHeightAction;
    ...
    private static void registerActions() {
        ...
        applyHeightAction = new ApplyHeightAction();
        JMenuItem heightItem = MainMenu.add(pluginMenu, applyHeightAction);
    ...
    private static final class BuildingKeyDispatcher implements KeyEventDispatcher {
        ...
        public boolean dispatchKeyEvent(KeyEvent event) {
            ...
            if (keyCode == KeyEvent.VK_Q
                    ...
                    && !event.isAltDown()
                    && !event.isMetaDown()
                    && applyHeightAction != null) {
                applyHeightAction.actionPerformed(null);
                return true;
            }
    ...
    private static final class ApplyHeightAction extends BuildingSelectionAction {
        ApplyHeightAction() {
            super(
                    "Set height from building:levels",
                    "Set height=3.6*building:levels on selected objects",
                    Shortcut.registerShortcut(
                            "buildingtagshortcuts:height-from-levels",
                            "Building Tag Shortcuts: Set height from building:levels",
                            KeyEvent.VK_Q,
                            Shortcut.CTRL_SHIFT),
                    "buildingtagshortcuts:height-from-levels");
        }
    ...
    private static void applyHeightFromLevels(BuildingSelectionAction support) {
        Collection<OsmPrimitive> selection = support.getSelection();
        if (selection == null || !support.confirmOutlying(selection)) {
            return;
        }
        ...
        for (OsmPrimitive primitive : selection) {
            ...
            commands.add(changeTag(primitive, HEIGHT, formatHeight(DEFAULT_LEVEL_HEIGHT * levels)));
            changed++;
        }
        ...
        if (skippedInvalid > 0) {
            message += " Skipped " + skippedInvalid + " invalid building:levels value(s).";
        }
        showInfo(message);
    }
